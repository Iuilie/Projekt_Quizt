    ...
    <div class="box-container">
        ...
        if($result_likes->num_rows > 0){
            while($fetch_likes = $result_likes->fetch_assoc()){
                ...
                $select_posts->bind_param("i", $fetch_likes['post_id']);
                    ...
                    while($fetch_posts = $result_posts->fetch_assoc()){
                        if($fetch_posts['status'] != 'deactive'){
                            ...
                            <form class="box" method="post">
                                ...
                                <a href="view_post.php?post_id=<?= htmlspecialchars($post_id); ?>" class="inline-btn">read more</a>
                                <div class="icons">
                                    <a href="view_post.php?post_id=<?= htmlspecialchars($post_id); ?>"><i class="fas fa-comment"></i><span>(<?= $total_post_likes; ?>)</span></a>
                                    <button type="submit" name="like_post"><i class="fas fa-heart" style="<?= ($total_post_likes > 0 && $user_id != '') ? 'color:red;' : ''; ?>"></i><span>(<?= $total_post_likes; ?>)</span></button>
                                </div>
                            </form>
                            <?php
                        }
